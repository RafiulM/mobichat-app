...
pub fn run() {
    ...
    #[cfg(feature = "tts")]
    {
        app_builder = app_builder.plugin(tauri_plugin_tts::init());
    }
        ...
        if let RunEvent::Exit = event {
            ...
            tokio::task::block_in_place(|| {
                tauri::async_runtime::block_on(async {
                    ...
                    #[cfg(feature = "tts")]
                    {
                        use tauri_plugin_tts::cleanup_tts_process;
                        if let Err(e) = cleanup_tts_process(app_handle.clone()).await {
                            log::warn!("Failed to cleanup TTS process: {}", e);
                        } else {
                            log::info!("TTS process cleaned up successfully");
                        }
                    }
